...
- Un palier se lit « **à partir du tour N inclus**, une victoire vaut V… », et c'est le palier au **plus grand seuil atteint** qui s'applique. Les points de BASE décrivent donc le cas normal (le match se décide tôt) et un palier la dégradation quand la rencontre traîne : base 3000, « à partir du tour 13 » → 2000. ⚠️ Le sens inverse (« jusqu'au tour N ») avait été implémenté d'abord et se lisait à l'envers — corrigé par la migration `RenommerPalierAPartirDuTour` (simple `RenameColumn`). Le seuil se compare à `MatchSheet.NombreDeTours`, **nullable** : non renseigné (feuilles antérieures) ⇒ points de base, jamais une exception ni un zéro. Le champ n'est proposé sur la feuille que si la ligue a des paliers.
...
**Correction d'une feuille par un commissaire** (`Matchs/Detail.razor`) : l'écran reprend **le même rendu que la saisie** (cartes à steppers, composant partagé `SelecteurJoueurMatch`, champ XP manuel, gains saisis) — une table à 11 colonnes était illisible et le commissaire avait deux interfaces à connaître. On peut **ajouter et retirer des joueurs**, pas seulement éditer les lignes existantes : un joueur oublié à la saisie ne recevait sinon jamais son XP et ses actions n'entraient pas au classement. `ModifierFeuilleAsync` **inverse** l'ancienne feuille (points, PSP, gains, fans, blessures, améliorations) puis réécrit tout — l'XP d'un joueur ajouté est donc créditée, un joueur déjà présent n'est pas compté deux fois, et corriger les gains ne reporte que l'**écart** sur la trésorerie.

⚠️ Deux pièges de ce chemin : à l'ouverture on ne « pince » `XpSaisie` que sur les lignes dont la valeur en base **diverge** du barème (sinon aucune ligne ne suivrait plus les steppers), et `PspGagnes` doit partir explicitement au service — son repli `if (PspGagnes <= 0)` recalculerait sinon et ignorerait une saisie manuelle.

**Une correction ANNULE l'après-match des DEUX coaches** (améliorations, XP dépensée restituée, blessures) et rouvre le match en `ValidationCompetences`. Le coach doit le savoir, sinon il retrouve un écran vierge sans comprendre : `MatchSheet.CorrigeeLe` + `ScoreAvantCorrection*` alimentent un **bandeau d'avertissement** sur `Matchs/ApresMatch.razor` (ancien score → nouveau, notes du commissaire) et un **mail dédié** « Votre feuille de match a été corrigée » — surtout pas le mail « Match confirmé », qui laissait croire à une notification déjà reçue.
